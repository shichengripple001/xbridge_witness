                    ...
                    if prev_lines:
                        ...
                        if not module or log_line.module == module:
                            if as_list:
                                ...
                                if pure_json:
                                    print(log_line.to_pure_json_str(), file=out)
                                else:
                                    print(log_line.to_mixed_json_str(), file=out)
                    prev_lines = l
                else:
                    if not prev_lines:
            ...
            if prev_lines:
                ...
                if not module or log_line.module == module:
                    if as_list:
                        ...
                        if pure_json:
                            print(log_line.to_pure_json_str(), file=out, flush=True)
                        else:
                            print(log_line.to_mixed_json_str(), file=out, flush=True)
    except Exception as e:
        eprint(f"Excption: {e}")
        raise e
    return result
    ...
    try:
        ...
        with open(args.output, "w") as out:
            if os.path.isdir(args.input):
                ...
                r = convert_log(args.input, out, pure_json=True)
